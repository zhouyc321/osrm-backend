                                 ...
                                 const double desired_segment_length)
{
    ...
    const auto getFactor = [desired_segment_length](const double first_distance, const double second_distance) {
...
getRepresentativeCoordinate(const NodeID from_node,
                            ...
                            const std::vector<extractor::QueryNode> &query_nodes,
                            const double desired_segment_length = detail::DESIRED_SEGMENT_LENGTH)
{
    const auto extractCoordinateFromNode =
        [](const extractor::QueryNode &node) -> util::Coordinate {
        return {node.lon, node.lat};
    ...
    else
    {
        ...
        if (traverse_in_reverse)
            return detail::getCoordinateFromCompressedRange(
                base_coordinate, geometry.rbegin(), geometry.rend(), final_coordinate, query_nodes, desired_segment_length);
        else
            return detail::getCoordinateFromCompressedRange(
                base_coordinate, geometry.begin(), geometry.end(), final_coordinate, query_nodes, desired_segment_length);
    }
}
